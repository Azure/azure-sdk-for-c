    ...
    az_span* out_mqtt_user_name)
{
  ...
  AZ_RETURN_IF_FAILED(
     az_iot_hub_client_user_name_get(&client->_internal.iot_hub_client, mqtt_user_name, &mqtt_user_name));
  AZ_RETURN_IF_FAILED(
      az_span_append_uint8(mqtt_user_name, pnp_client_param_separator, &mqtt_user_name));
  AZ_RETURN_IF_FAILED(
      az_span_append(mqtt_user_name, pnp_model_id, &mqtt_user_name));
  AZ_RETURN_IF_FAILED(
