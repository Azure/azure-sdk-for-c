    ...
    az_span* out_mqtt_topic_filter)
{
  ...
  AZ_RETURN_IF_FAILED(az_span_append(mqtt_topic_filter, methods_topic_prefix, &mqtt_topic_filter));
  AZ_RETURN_IF_FAILED(az_span_append(mqtt_topic_filter, methods_topic_filter_suffix, &mqtt_topic_filter));
    ...
    az_iot_hub_client_method_request* out_request)
{
  ...
  received_topic = az_span_slice(received_topic, index + az_span_length(methods_topic_prefix), az_span_length(received_topic));
  ...
  received_topic = az_span_slice(received_topic, index + az_span_length(methods_topic_filter_suffix), az_span_length(received_topic));
  ...
  out_request->request_id = az_span_slice(received_topic, index + az_span_length(methods_response_topic_properties), az_span_length(received_topic));
    ...
    az_span* out_mqtt_topic)
{
  ...
  AZ_RETURN_IF_FAILED(az_span_append(mqtt_topic, methods_topic_prefix, &mqtt_topic));
