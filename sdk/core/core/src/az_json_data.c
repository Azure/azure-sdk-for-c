...
_az_span_aligned_append(az_span * const builder, _az_data const data, void const ** out) {
  // alignment.
  {
...
AZ_NODISCARD az_result _az_span_top_aligned_append(az_span * const builder, _az_data data) {
  ...
  uint8_t * end = _az_align_floor(begin + size - data.type.size, data.type.align);
  ...
  switch (token.kind) {
    ...
    case AZ_JSON_TOKEN_OBJECT: {
      ...
      while (true) {
        ...
        az_result result = az_json_parser_parse_token_member(parser, &token_member);
    ...
    case AZ_JSON_TOKEN_ARRAY: {
      ...
      while (true) {
        ...
        az_result result = az_json_parser_parse_array_item(parser, &item_token);
        if (result == AZ_ERROR_ITEM_NOT_FOUND) {
          break;
        }
        AZ_RETURN_IF_FAILED(result);
