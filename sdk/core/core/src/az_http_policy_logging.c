...
static az_result _az_http_policy_logging_log_value_msg(
...
static az_result _az_http_policy_logging_log_http_request_msg(
    az_span_builder * const log_msg_bldr,
  ...
  for (uint16_t i = 0; i < headers_end; ++i) {
    ...
    if (!az_span_is_empty(header.value)) {
      AZ_RETURN_IF_FAILED(az_span_builder_append(log_msg_bldr, AZ_STR(" : ")));
      AZ_RETURN_IF_FAILED(_az_http_policy_logging_log_value_msg(log_msg_bldr, header.value));
    }
  }
...
AZ_INLINE az_result _az_http_policy_logging_log_http_response_msg(
    az_span_builder * const log_msg_bldr,
  ...
  AZ_RETURN_IF_FAILED(az_span_builder_append(log_msg_bldr, AZ_STR("\n\n")));
  return _az_http_policy_logging_log_http_request_msg(log_msg_bldr, hrb, 1);
}
...
void _az_http_policy_logging_log_http_request(az_http_request_builder const * const hrb) {
  ...
  (void)_az_http_policy_logging_log_http_request_msg(&log_msg_bldr, hrb, 0);
...
void _az_http_policy_logging_log_http_response(
  ...
  (void)_az_http_policy_logging_log_http_response_msg(&log_msg_bldr, response, duration_msec, hrb);

  az_log_write(AZ_LOG_HTTP_RESPONSE, az_span_builder_result(&log_msg_bldr));
}

AZ_NODISCARD az_result _az_http_policy_logging(az_http_policy_callback next_policy) {
  ...
  if (!az_log_should_write(AZ_LOG_HTTP_RESPONSE)) {
    ...
    return next_http_policy(p_policies, hrb, response);
  }

  uint64_t const start = _az_clock_msec();
  az_result const result = next_http_policy(p_policies, hrb, response);
  uint64_t const end = _az_clock_msec();

  _az_http_policy_logging_log_http_response(response, end - start, hrb);
