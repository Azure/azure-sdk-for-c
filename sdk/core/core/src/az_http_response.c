...
static AZ_NODISCARD az_result _az_get_digit(az_span* self, uint8_t* save_here)
{
  ...
  *self = az_span_slice_start(*self, 1);
...
_az_get_http_status_line(az_span* self, az_http_response_status_line* out_status_line)
{
  ...
  // status-code = 3DIGIT
  {
    ...
    // move reader
    *self = az_span_slice_start(*self, 3);
  }
  ...
  *self = az_span_slice_start(*self, offset + 1);
  // CR LF
...
az_http_response_get_next_header(az_http_response* response, az_pair* out_header)
{
  ...
  // field-name     = token
  {
    ...
    *reader = az_span_slice_start(*reader, field_name_length + 1);

    // OWS
    int32_t ows_len = 0;
    AZ_RETURN_IF_FAILED(_az_scan_until(*reader, _az_slice_is_not_http_whitespace, &ows_len));
    *reader = az_span_slice_start(*reader, ows_len);
  }
  ...
  // Note: obs-fold is not implemented.
  {
    ...
    *reader = az_span_slice_start(*reader, offset);
  }
...
AZ_NODISCARD az_result az_http_response_get_body(az_http_response* response, az_span* out_body)
{
  ...
  *out_body = az_span_slice_start(response->_internal.parser.remaining, 0);
