...
void test_json_builder() {
  {
    ...
    TEST_EXPECT_SUCCESS(az_json_builder_write(&builder, az_json_token_object()));

    TEST_EXPECT_SUCCESS(az_json_builder_write_object_member(
        &builder, AZ_SPAN_FROM_STR("name"), az_json_token_boolean(true)));

    {
      TEST_EXPECT_SUCCESS(az_json_builder_write_object_member(
          &builder, AZ_SPAN_FROM_STR("foo"), az_json_token_array()));
      TEST_EXPECT_SUCCESS(az_json_builder_write_array_item(
          &builder, az_json_token_string(AZ_SPAN_FROM_STR("bar"))));
      TEST_EXPECT_SUCCESS(az_json_builder_write_array_item(&builder, az_json_token_null()));
      TEST_EXPECT_SUCCESS(az_json_builder_write_array_item(&builder, az_json_token_number(0)));
      TEST_EXPECT_SUCCESS(az_json_builder_write_array_item(&builder, az_json_token_number(-12)));
      TEST_EXPECT_SUCCESS(az_json_builder_write_array_close(&builder));
    }
    ...
    TEST_EXPECT_SUCCESS(az_json_builder_write_object_close(&builder));
