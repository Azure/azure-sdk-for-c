...
void test_url_parse() {
  {
    ...
    TEST_ASSERT(az_span_eq(url.authority.port, AZ_STR("81")));
  ...
  }
  {
    ...
    TEST_ASSERT(az_span_eq(url.scheme, AZ_STR("https")));
    TEST_ASSERT(az_span_eq(url.authority.userinfo, AZ_STR("")));
    TEST_ASSERT(az_span_eq(url.authority.host, AZ_STR("example.com.localhost")));
    TEST_ASSERT(az_span_eq(url.authority.port, AZ_STR("")));
  ...
  }
  {
    ...
    TEST_ASSERT(az_span_eq(url.scheme, AZ_STR("https")));
    TEST_ASSERT(az_span_eq(url.authority.userinfo, AZ_STR("")));
    TEST_ASSERT(az_span_eq(url.authority.host, AZ_STR("sub-domain.example.com")));
    TEST_ASSERT(az_span_eq(url.authority.port, AZ_STR("")));
