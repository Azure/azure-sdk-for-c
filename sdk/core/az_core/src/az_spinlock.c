...
void _az_spinlock_enter_writer(_az_spinlock* lock)
{
  while (true)
  {
    ...
    if ((state == 0 || state == _az_MASK_WRITER_WAITING_BIT)
        && az_platform_atomic_compare_exchange(
            &lock->_internal.state, state, (uint32_t)_az_MASK_WRITER_BIT))
    {
      return;
    }
    ...
    if ((state & _az_MASK_WRITER_WAITING_BIT) == 0
        && az_platform_atomic_compare_exchange(
            &lock->_internal.state, state, state | _az_MASK_WRITER_WAITING_BIT))
    {
      return;
    }
  }
...
void _az_spinlock_exit_writer(_az_spinlock* lock)
{
  ...
  lock->_internal.state = lock->_internal.state & _az_MASK_WRITER_WAITING_BIT;
}
...
void _az_spinlock_enter_reader(_az_spinlock* lock)
{
  while (true)
  {
    ...
    if ((state & _az_MASK_WRITER_BITS) == 0)
    {
...
void _az_spinlock_exit_reader(_az_spinlock* lock)
{
  while (true)
  {
    ...
    if (az_platform_atomic_compare_exchange(
            ...
            state,
            ((state & _az_MASK_READER_BITS) - 1) | (state & _az_MASK_WRITER_WAITING_BIT)))
    {
      return;
    }
  }
