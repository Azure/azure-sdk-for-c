...
#define TEST_CSR "this is a mock CSR"
...
static void test_az_iot_provisioning_client_register_get_request_payload_csr_succeed(
    void** state)
{
  (void)state;
  ...
  ret = az_iot_provisioning_client_register_get_request_payload(
      &client, AZ_SPAN_EMPTY, &options, payload, sizeof(payload), &payload_len);
  _az_POP_WARNINGS
  assert_int_equal(AZ_OK, ret);
  assert_int_equal(payload_len, expected_payload_len);
  assert_memory_equal(expected_payload, payload, expected_payload_len);
...
int test_az_iot_provisioning_client_register_get_request_payload()
{
  ...
  const struct CMUnitTest tests[] = {
    ...
    cmocka_unit_test(test_az_iot_provisioning_client_register_get_request_payload_csr_succeed),
  };
