...
static void test_json_builder_chunked(void** state)
{
  (void)state;
  {
    az_json_builder builder = { 0 };
    ...
    assert_true(az_span_is_content_equal(AZ_SPAN_NULL, az_json_builder_get_json(&builder)));

    uint8_t array[200] = { 0 };
    az_span_to_str(
        (char*)array,
        200,
        az_span_slice(AZ_SPAN_FROM_BUFFER(json_array), 0, builder._internal.total_bytes_written));
  ...
  }
  {
...

    {
      ...
      assert_true(
          az_span_is_content_equal(AZ_SPAN_NULL, az_json_builder_get_json(&nested_object_builder)));
...
static void test_json_builder_chunked_no_callback(void** state)
{
  (void)state;
  {
    uint8_t array[200] = { 0 };
...
int test_az_json()
{
  const struct CMUnitTest tests[] = { cmocka_unit_test(test_json_parser_init),
                                      cmocka_unit_test(test_json_builder),
                                      cmocka_unit_test(test_json_builder_chunked),
                                      cmocka_unit_test(test_json_builder_chunked_no_callback),
                                      cmocka_unit_test(test_json_parser),
                                      cmocka_unit_test(test_json_parser_invalid),
