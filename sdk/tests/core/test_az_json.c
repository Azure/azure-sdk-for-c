...
{
  // Write some JSON into a buffer
  uint8_t array[200] = { 0 };
  az_json_writer writer = { 0 };

  az_json_writer_init(&writer, AZ_SPAN_FROM_BUFFER(array), NULL);

  az_json_writer_append_begin_object(&writer);
  az_json_writer_append_property_name(&writer, AZ_SPAN_FROM_STR("name"));
  az_json_writer_append_bool(&writer, true);

  az_json_writer_append_property_name(&writer, AZ_SPAN_FROM_STR("foo"));
  az_json_writer_append_begin_array(&writer);
  az_json_writer_append_string(&writer, AZ_SPAN_FROM_STR("bar"));
  az_json_writer_append_int32(&writer, -12);
  az_json_writer_append_end_array(&writer);

  az_json_writer_append_end_object(&writer);

  // Get the resulting JSON created: {"name":true,"foo":["bar",-12]}
  az_span output_json = az_json_writer_get_bytes_used_in_destination(&writer);

  // Read and parse some JSON to look for a particular property and integer value.
  az_json_reader reader = { 0 };
  // We also have chunked init that accepts array of non-contiguous buffers
  az_json_reader_init(&reader, output_json, NULL);
  ...
  while (az_result_succeeded(az_json_reader_next_token(reader)))
  {
    if (reader.token.kind == AZ_JSON_TOKEN_PROPERTY_NAME
        && (az_json_token_is_text_equal(&reader.token, AZ_SPAN_FROM_STR("foo"))
    {
      // Found a property, corresponding to the array, find and extract the integers.
      az_json_token property_name = reader.token;
      az_json_reader_next_token(reader); // Read the start of array

      while (az_result_succeeded(az_json_reader_next_token(reader))
             && reader.token.kind != AZ_JSON_TOKEN_END_ARRAY)
      {
        if (reader.token.kind == AZ_JSON_TOKEN_NUMBER)
        {
          az_json_token_get_int32(&reader.token, &result);
        }
      }
    }
    ...
    {
      // Ignore sub-objects
      az_json_reader_skip_children(reader);
    }
  ...
  // result at this point should be -12
