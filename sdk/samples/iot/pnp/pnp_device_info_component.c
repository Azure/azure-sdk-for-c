...
az_result pnp_device_info_build_reported_property(az_span payload, az_span* out_payload)
{
  ...
  IOT_SAMPLE_RETURN_IF_FAILED(az_json_writer_append_property_name(&jw, manufacturer_property_name));
  IOT_SAMPLE_RETURN_IF_FAILED(az_json_writer_append_string(&jw, manufacturer_property_value));
  IOT_SAMPLE_RETURN_IF_FAILED(az_json_writer_append_property_name(&jw, model_property_name));
  IOT_SAMPLE_RETURN_IF_FAILED(az_json_writer_append_string(&jw, model_property_value));
  IOT_SAMPLE_RETURN_IF_FAILED(
      az_json_writer_append_property_name(&jw, software_version_property_name));
  IOT_SAMPLE_RETURN_IF_FAILED(az_json_writer_append_string(&jw, software_version_property_value));
  IOT_SAMPLE_RETURN_IF_FAILED(az_json_writer_append_property_name(&jw, os_name_property_name));
  IOT_SAMPLE_RETURN_IF_FAILED(az_json_writer_append_string(&jw, os_name_property_value));
  IOT_SAMPLE_RETURN_IF_FAILED(
      az_json_writer_append_property_name(&jw, processor_architecture_property_name));
  IOT_SAMPLE_RETURN_IF_FAILED(
      az_json_writer_append_string(&jw, processor_architecture_property_value));
  IOT_SAMPLE_RETURN_IF_FAILED(
      az_json_writer_append_property_name(&jw, processor_manufacturer_property_name));
  IOT_SAMPLE_RETURN_IF_FAILED(
      az_json_writer_append_string(&jw, processor_manufacturer_property_value));
  IOT_SAMPLE_RETURN_IF_FAILED(
      az_json_writer_append_property_name(&jw, total_storage_property_name));
  IOT_SAMPLE_RETURN_IF_FAILED(
      az_json_writer_append_double(&jw, total_storage_property_value, DOUBLE_DECIMAL_PLACE_DIGITS));
  IOT_SAMPLE_RETURN_IF_FAILED(az_json_writer_append_property_name(&jw, total_memory_property_name));
  IOT_SAMPLE_RETURN_IF_FAILED(
