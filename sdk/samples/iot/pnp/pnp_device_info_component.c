...
void pnp_device_info_build_reported_property(az_span payload, az_span* out_payload)
{
  ...
  iot_sample_error_log_init(&log, "Failed to build reported property payload for device info: az_result return code 0x%08x.", AZ_SPAN_EMPTY);
  ...
  IOT_SAMPLE_EXIT_IF_FAILED(az_json_writer_append_string(&jw, manufacturer_property_value), log);
  IOT_SAMPLE_EXIT_IF_FAILED(az_json_writer_append_property_name(&jw, model_property_name), log);
  IOT_SAMPLE_EXIT_IF_FAILED(az_json_writer_append_string(&jw, model_property_value), log);
  IOT_SAMPLE_EXIT_IF_FAILED(
  ...
  IOT_SAMPLE_EXIT_IF_FAILED(az_json_writer_append_property_name(&jw, os_name_property_name), log);
  IOT_SAMPLE_EXIT_IF_FAILED(az_json_writer_append_string(&jw, os_name_property_value), log);
  IOT_SAMPLE_EXIT_IF_FAILED(
