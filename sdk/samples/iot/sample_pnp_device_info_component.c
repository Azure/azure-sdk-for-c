...
bool pnp_device_info_get_report_data(
    const az_iot_hub_client* client,
    pnp_mqtt_message* mqtt_message)
{
  az_json_writer jw;
  if (az_failed(az_json_writer_init(&jw, mqtt_message->payload_span, NULL))
      ...
      || az_failed(az_json_writer_append_string(&jw, pnp_device_info_manufacturer_property_value))
      || az_failed(az_json_writer_append_property_name(&jw, pnp_device_info_model_property_name))
      || az_failed(az_json_writer_append_string(&jw, pnp_device_info_model_property_value))
      || az_failed(
      ...
      || az_failed(az_json_writer_append_property_name(&jw, pnp_device_info_os_name_property_name))
      || az_failed(az_json_writer_append_string(&jw, pnp_device_info_os_name_property_value))
      || az_failed(az_json_writer_append_property_name(
  ...
  return az_succeeded(az_iot_hub_client_twin_patch_get_publish_topic(
      client,
      get_request_id(),
      mqtt_message->topic,
