...
// Functions
static void create_and_configure_provisioning_mqtt_client(void);
...
int main(void)
{
  create_and_configure_provisioning_mqtt_client();
  ...
  create_and_configure_iot_hub_mqtt_client();
  IOT_SAMPLE_LOG_SUCCESS("Client created and configured.");

  connect_mqtt_client_to_iot_hub();
  IOT_SAMPLE_LOG_SUCCESS("Client connected to IoT Hub.\n");

  disconnect_mqtt_client_from_iot_hub_service();
  IOT_SAMPLE_LOG_SUCCESS("Client disconnected from IoT Hub service.");
...
static void create_and_configure_provisioning_mqtt_client(void)
{
  ...
  rc = MQTTClient_create(
      &dps_mqtt_client, mqtt_endpoint_buffer, mqtt_client_id_buffer, MQTTCLIENT_PERSISTENCE_NONE, NULL);
...
static void connect_mqtt_client_to_provisioning_service(void)
{
  ...
  rc = MQTTClient_connect(dps_mqtt_client, &mqtt_connect_options);
  if (rc != MQTTCLIENT_SUCCESS)
  {
    IOT_SAMPLE_LOG_ERROR(
...
static void subscribe_mqtt_client_to_provisioning_service_topics(void)
{
  ...
  int rc
      = MQTTClient_subscribe(dps_mqtt_client, AZ_IOT_PROVISIONING_CLIENT_REGISTER_SUBSCRIBE_TOPIC, 1);
  if (rc != MQTTCLIENT_SUCCESS)
  {
    IOT_SAMPLE_LOG_ERROR(
...
static void register_device_with_provisioning_service(void)
{
  ...
  rc = MQTTClient_publishMessage(dps_mqtt_client, register_topic_buffer, &pubmsg, NULL);
  if (rc != MQTTCLIENT_SUCCESS)
  {
...
static void receive_device_registration_status_message(void)
{
  ...
  do
  {
    ...
    int rc = MQTTClient_receive(dps_mqtt_client, &topic, &topic_len, &message, MQTT_TIMEOUT_RECEIVE_MS);
...
static void disconnect_mqtt_client_from_provisioning_service(void)
{
  int rc = MQTTClient_disconnect(dps_mqtt_client, MQTT_TIMEOUT_DISCONNECT_MS);
  if (rc != MQTTCLIENT_SUCCESS)
  {
  ...
  MQTTClient_destroy(&dps_mqtt_client);
}
    ...
    az_iot_provisioning_client_register_response const* register_response)
{
  ...
  rc = MQTTClient_publish(
      dps_mqtt_client, query_topic_buffer, 0, NULL, IOT_SAMPLE_MQTT_PUBLISH_QOS, 0, NULL);
  if (rc != MQTTCLIENT_SUCCESS)
  {
...
static void create_and_configure_iot_hub_mqtt_client(void)
{
  ...
  // Create the Paho MQTT client.
  rc = MQTTClient_create(
      &hub_mqtt_client, mqtt_endpoint_buffer, mqtt_client_id_buffer, MQTTCLIENT_PERSISTENCE_NONE, NULL);
  if (rc != MQTTCLIENT_SUCCESS)
  {
    IOT_SAMPLE_LOG_ERROR("Failed to create MQTT client: MQTTClient return code %d.", rc);
    exit(rc);
...
static void connect_mqtt_client_to_iot_hub(void)
{
  ...
  rc = MQTTClient_connect(hub_mqtt_client, &mqtt_connect_options);
  if (rc != MQTTCLIENT_SUCCESS)
  {
    IOT_SAMPLE_LOG_ERROR(
        "Failed to connect: MQTTClient return code %d.\n"
...
static void disconnect_mqtt_client_from_iot_hub_service(void)
{
  int rc = MQTTClient_disconnect(hub_mqtt_client, MQTT_TIMEOUT_DISCONNECT_MS);
  if (rc != MQTTCLIENT_SUCCESS)
  {
    IOT_SAMPLE_LOG_ERROR("Failed to disconnect MQTT client: MQTTClient return code %d.", rc);
    exit(rc);
...
#else // _WIN32
  file = fopen((char*)az_span_ptr(file_path), "w");
