...
/*
DESCRIPTION
This sample utilizes MQTT implementation provided by Mosquitto and shows how to
use "azure sdk for embedded c" to connect to Azure IoT Hub as a client.
...
3. responding to Method-Calls from Azure IoT Hub
...
/* shared access key duration time in seconds */
...
static int mqttVer = MQTT_PROTOCOL_V311;
...
/*******************************************************************************
 *
 * mqttPublish - publish a message to Azure IoT Hub
 *
...
/*******************************************************************************
 *
 * methodRespSend - send a response for a method call
 *
...
)
{
  ...
  if (az_result_succeeded(az_iot_hub_client_methods_response_get_publish_topic(
          azureClient, req->request_id, status, respTopic, sizeof(respTopic), NULL)))
  {
    printf_s("az_iot_hub_client_methods_response_get_publish_topic OK!\n");
  }
  else
  {
...
)
{
  ...
  {
    testMethod();
    methodRespSend(mqttClient, azureClient, methodReq);
  }
}
 ...
 *
 * This routine generates a reported property payload in JSON format.
 *
 * ERRNO: N/A
 *
...
)
{
  ...
  if (az_result_succeeded(az_iot_hub_client_twin_patch_get_publish_topic(
          azureClient, twinPatchRequestId, twinPatchTopic, sizeof(twinPatchTopic), NULL)))
  {
    printf_s("az_iot_hub_client_twin_patch_get_publish_topic OK!\n");
  }
  else
  {
...
)
{
  ...
  /* stop the sample as soon as 3 C2D messages are received */
  if (received == 3)
  {
 ...
 *
 * This routine subscribes to MQTT topics:
 * 1. C2D message
 * 2. methods-call
...
static bool deviceTwinDocumentGet(struct mosquitto* mqttClient, az_iot_hub_client* azureClient)
{
  ...
  if (az_result_succeeded(az_iot_hub_client_twin_document_get_publish_topic(
          azureClient, twinDocRequestId, twinDocGetTopic, sizeof(twinDocGetTopic), NULL)))
  {
    printf_s("az_iot_hub_client_twin_document_get_publish_topic OK!\n");
  }
  else
  {
...
static bool telemetrySend(struct mosquitto* mqttClient, az_iot_hub_client* azureClient)
{
  ...
  if (az_result_succeeded(az_iot_hub_client_telemetry_get_publish_topic(
          azureClient, NULL, telemetryTopic, sizeof(telemetryTopic), NULL)))
  {
    printf_s("az_iot_hub_client_telemetry_get_publish_topic OK!\n");
  }
  else
  {
 ...
 * 3. It exits when receiving 3 C2D
 * messages.
 *
 * ERRNO: N/A
 *
