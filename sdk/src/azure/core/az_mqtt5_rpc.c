...
AZ_NODISCARD int32_t _az_rpc_calculate_substitution_length(az_span format, az_span key, az_span value);
AZ_NODISCARD az_result _az_rpc_substitute_key_for_value(az_span format, uint8_t* format_buf, az_span key, az_span value, az_span out_topic);
...
AZ_NODISCARD az_result _az_rpc_substitute_key_for_value(az_span format, uint8_t* format_buf, az_span key, az_span value, az_span out_topic)
{
  ...
  if (index >= 0)
  {
    ...
    temp_span = az_span_copy(
        temp_span, az_span_slice_to_end(temp_format_buf, index + az_span_size(key)));
    return AZ_OK;
  }
    ...
    int32_t* out_topic_length)
{
  ...
  // Determine the length of the final topic and the max length of the topic while performing substitutions
  int32_t topic_length = format_size + 1; // + 1 for the null terminator
  int32_t max_temp_length = 0;
  if ((topic_length += _az_rpc_calculate_substitution_length(format, model_id_key, model_id)) > max_temp_length)
  {
    max_temp_length = topic_length;
  }
  if ((topic_length += _az_rpc_calculate_substitution_length(format, command_name_key, command_name)) > max_temp_length)
  {
    max_temp_length = topic_length;
  }
  if ((topic_length += _az_rpc_calculate_substitution_length(format, executor_client_id_key, executor_client_id)) > max_temp_length)
  {
    max_temp_length = topic_length;
  }
  if ((topic_length += _az_rpc_calculate_substitution_length(format, invoker_client_id_key, invoker_client_id)) > max_temp_length)
  {
    max_temp_length = topic_length;
  }
  ...
  if (az_result_succeeded(_az_rpc_substitute_key_for_value(format, format_buf, model_id_key, model_id, out_topic)))
  {
  ...
  if (az_result_succeeded(_az_rpc_substitute_key_for_value(format, format_buf, command_name_key, command_name, out_topic)))
  {
  ...
  if (az_result_succeeded(_az_rpc_substitute_key_for_value(format, format_buf, executor_client_id_key, executor_client_id, out_topic)))
  {
  ...
  if (az_result_succeeded(_az_rpc_substitute_key_for_value(format, format_buf, invoker_client_id_key, invoker_client_id, out_topic)))
  {
