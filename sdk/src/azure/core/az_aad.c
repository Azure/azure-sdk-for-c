...
{
  _az_RETURN_IF_NOT_ENOUGH_SIZE(url, az_span_size(authority));
  az_span remainder = az_span_copy(url, authority);

  {
    int32_t url_length = 0;
    _az_RETURN_IF_FAILED(_az_span_url_encode(remainder, tenant_id, &url_length));
    remainder = az_span_slice_to_end(remainder, url_length);
  }

  az_span const oath_token = AZ_SPAN_FROM_STR("/oauth2/v2.0/token");
  _az_RETURN_IF_NOT_ENOUGH_SIZE(remainder, az_span_size(oath_token));
    ...
    az_span* out_body)
{
  ...
  _az_RETURN_IF_NOT_ENOUGH_SIZE(body, az_span_size(grant_type_and_client_id_key));

  az_span remainder = az_span_copy(body, grant_type_and_client_id_key);
  int32_t body_length = 0;

  _az_RETURN_IF_FAILED(_az_span_url_encode(remainder, client_id, &body_length));
  remainder = az_span_slice_to_end(remainder, body_length);

  az_span const scope_key = AZ_SPAN_FROM_STR("&scope=");
  _az_RETURN_IF_NOT_ENOUGH_SIZE(remainder, az_span_size(scope_key));

  remainder = az_span_copy(remainder, scope_key);

  _az_RETURN_IF_FAILED(_az_span_url_encode(remainder, scopes, &body_length));
  remainder = az_span_slice_to_end(remainder, body_length);

  if (az_span_size(client_secret) > 0)
  {
    az_span const client_secret_key = AZ_SPAN_FROM_STR("&client_secret=");
    _az_RETURN_IF_NOT_ENOUGH_SIZE(remainder, az_span_size(client_secret_key));

    remainder = az_span_copy(remainder, client_secret_key);

    _az_RETURN_IF_FAILED(_az_span_url_encode(remainder, client_secret, &body_length));
    remainder = az_span_slice_to_end(remainder, body_length);
  }
...
{
  az_json_reader jr;
  _az_RETURN_IF_FAILED(az_json_reader_init(&jr, body, NULL));
  ...
  while (jr.token.kind != AZ_JSON_TOKEN_END_OBJECT)
  {
    ...
    {
      _az_RETURN_IF_FAILED(az_json_reader_next_token(&jr));
      _az_RETURN_IF_FAILED(az_json_token_get_int64(&jr.token, expires_in_seconds));
...
AZ_NODISCARD az_result _az_aad_request_token(az_http_request* request, _az_token* out_token)
{
  ...
  _az_RETURN_IF_FAILED(az_http_response_init(&response, AZ_SPAN_FROM_BUFFER(response_buf)));
  ...
  _az_RETURN_IF_FAILED(az_http_pipeline_process(&pipeline, request, &response));

  // If we failed to get the token, we return failure/
  az_http_response_status_line status_line = { 0 };
  _az_RETURN_IF_FAILED(az_http_response_get_status_line(&response, &status_line));
  ...
  az_span body = { 0 };
  _az_RETURN_IF_FAILED(az_http_response_get_body(&response, &body));

  int64_t expires_in_seconds = 0;
  az_json_token json_access_token = { 0 };
  _az_RETURN_IF_FAILED(_az_parse_json_payload(body, &expires_in_seconds, &json_access_token));
