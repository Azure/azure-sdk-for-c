          ...
          this_policy->_internal.rpc_client->_internal.subscription_topic, recv_data->topic))
  {
    ...
    if (source_state != NULL)
    {
      ...
      _az_RETURN_IF_FAILED(_az_mqtt5_connection_api_callback(
          ...
          (az_event){ .type = AZ_EVENT_MQTT5_RPC_CLIENT_READY_IND,
...
static az_result root(az_event_policy* me, az_event event)
{
  ...
  switch (event.type)
  {
    ...
    case AZ_HFSM_EVENT_EXIT:
    {
      ...
      {
        _az_LOG_WRITE(AZ_HFSM_EVENT_EXIT, AZ_SPAN_FROM_STR("az_mqtt5_rpc_client_policy: PANIC!"));
      }
 ...
 */
AZ_INLINE az_result _rpc_start_timer(az_mqtt5_rpc_client_policy* me, uint32_t timeout_in_seconds)
{
  ...
  int32_t delay_milliseconds
      = (int32_t)timeout_in_seconds * 1000;
 ...
 */
AZ_INLINE az_result _rpc_stop_timer(az_mqtt5_rpc_client_policy* me)
{
  _az_event_pipeline_timer* timer = &me->_internal.rpc_client_timer;
...
static az_result idle(az_event_policy* me, az_event event)
{
  ...
  switch (event.type)
  {
    ...
    case AZ_EVENT_MQTT5_RPC_CLIENT_UNSUB_REQ:
    {
      ...
      _az_RETURN_IF_FAILED(unsubscribe(this_policy, NULL, NULL));
      break;
    }
...
static az_result subscribing(az_event_policy* me, az_event event)
{
  ...
  {
    case AZ_HFSM_EVENT_ENTRY:
      _rpc_start_timer(this_policy, this_policy->_internal.rpc_client->_internal.options.subscribe_timeout_in_seconds);
      break;
    ...
    case AZ_EVENT_MQTT5_RPC_CLIENT_UNSUB_REQ:
    {
      ...
      _az_RETURN_IF_FAILED(unsubscribe(this_policy, subscribing, idle));
      break;
    }
...
static az_result ready(az_event_policy* me, az_event event)
{
  ...
  switch (event.type)
  {
    ...
    case AZ_EVENT_MQTT5_RPC_CLIENT_SUB_REQ:
    {
      // already subscribed, application doesn't need to wait for AZ_EVENT_MQTT5_RPC_CLIENT_READY_IND to
      // invoke commands
    ...
    {
      // send unsubscribe and transition to idle if this request is for this policy
      _az_RETURN_IF_FAILED(unsubscribe(this_policy, ready, idle));
      break;
    }
...
static az_result publishing(az_event_policy* me, az_event event)
{
  ...
  {
    case AZ_HFSM_EVENT_ENTRY:
      _rpc_start_timer(this_policy, this_policy->_internal.rpc_client->_internal.options.publish_timeout_in_seconds);
      break;
    ...
    case AZ_MQTT5_EVENT_PUBACK_RSP:
    {
      ...
      if (puback_data->id == this_policy->_internal.pending_pub_id)
      {
        if (puback_data->puback_reason != 0)
        {
          ...
          _az_RETURN_IF_FAILED(_az_mqtt5_connection_api_callback(
              this_policy->_internal.connection,
              (az_event){ .type = AZ_EVENT_MQTT5_RPC_CLIENT_ERROR_RSP, .data = &resp_data }));
        }
    ...
    az_mqtt5_rpc_client_invoke_req_event_data* data)
{
  ...
  return _az_hfsm_send_event(
      ...
      (az_event){ .type = AZ_EVENT_MQTT5_RPC_CLIENT_INVOKE_REQ, .data = data });
}
...
AZ_NODISCARD az_result az_mqtt5_rpc_client_subscribe_begin(az_mqtt5_rpc_client_policy* client)
{
  ...
  return _az_hfsm_send_event(
      &client->_internal.rpc_client_hfsm,
      (az_event){ .type = AZ_EVENT_MQTT5_RPC_CLIENT_SUB_REQ, .data = NULL });
}
...
AZ_NODISCARD az_result az_mqtt5_rpc_client_unsubscribe_begin(az_mqtt5_rpc_client_policy* client)
{
  ...
  return _az_hfsm_send_event(
      &client->_internal.rpc_client_hfsm,
      (az_event){ .type = AZ_EVENT_MQTT5_RPC_CLIENT_UNSUB_REQ, .data = NULL });
}
