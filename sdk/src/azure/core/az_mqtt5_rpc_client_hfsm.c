    ...
    az_event_policy_handler source_state)
{
          ...
          this_policy->_internal.rpc_client->_internal.subscription_topic, recv_data->topic))
  {
    ...
    if (source_state != NULL)
    {
      ...
      // AZ_EVENT_MQTT5_RPC_CLIENT_READY_IND, .data = this_policy->_internal.rpc_client});
      // }
      _az_RETURN_IF_FAILED(_az_mqtt5_connection_api_callback(
          this_policy->_internal.connection,
...
static az_result subscribing(az_event_policy* me, az_event event)
{
  ...
  switch (event.type)
  {
    case AZ_HFSM_EVENT_ENTRY:
      _rpc_start_timer(
          this_policy,
          this_policy->_internal.rpc_client->_internal.options.subscribe_timeout_in_seconds);
      break;
...
send_resp_inbound_if_topic_matches(az_mqtt5_rpc_client_policy* this_policy, az_event event)
{
          ...
          this_policy->_internal.rpc_client->_internal.subscription_topic, recv_data->topic))
  {
    ...
    // {
    // az_event_policy_send_inbound_event((az_event_policy*)this_policy, (az_event){.type =
    // AZ_EVENT_MQTT5_RPC_SERVER_EXECUTE_COMMAND_REQ, .data = &resp_data});
    // }
...
static az_result ready(az_event_policy* me, az_event event)
{
  ...
  switch (event.type)
  {
    ...
    case AZ_EVENT_MQTT5_RPC_CLIENT_SUB_REQ:
    {
      ...
      // AZ_EVENT_MQTT5_RPC_CLIENT_READY_IND to invoke commands
...
static az_result publishing(az_event_policy* me, az_event event)
{
  ...
  switch (event.type)
  {
    case AZ_HFSM_EVENT_ENTRY:
      _rpc_start_timer(
          this_policy,
          this_policy->_internal.rpc_client->_internal.options.publish_timeout_in_seconds);
      break;
    ...
    case AZ_MQTT5_EVENT_PUBACK_RSP:
    {
      ...
      if (puback_data->id == this_policy->_internal.pending_pub_id)
      {
        if (puback_data->puback_reason != 0)
        {
          ...
          // {
          // az_event_policy_send_inbound_event((az_event_policy*)this_policy, (az_event){.type =
          // AZ_EVENT_MQTT5_RPC_CLIENT_ERROR_RSP, .data = &resp_data});
          // }
          _az_RETURN_IF_FAILED(_az_mqtt5_connection_api_callback(
              this_policy->_internal.connection,
