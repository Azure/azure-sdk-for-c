...
az_rpc_client_get_response_topic(az_mqtt5_rpc_client* client, az_span* out_response_topic)
{
  ...
  _az_PRECONDITION_VALID_SPAN(client->_internal.server_client_id, 1, false);
  int32_t response_topic_min_length = az_span_size(client->_internal.model_id)
      + az_span_size(client->_internal.client_id) + az_span_size(client->_internal.command_name)
      + az_span_size(client->_internal.server_client_id) + 27;
  ...
  temp_span = az_span_copy(temp_span, client->_internal.server_client_id);
...
az_rpc_client_get_request_topic(az_mqtt5_rpc_client* client, az_span out_request_topic)
{
  ...
  int32_t request_topic_min_length = az_span_size(client->_internal.model_id)
      + az_span_size(client->_internal.server_client_id) + az_span_size(client->_internal.command_name) + 23;
